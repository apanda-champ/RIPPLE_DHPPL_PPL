...
class PVPTD3(TD3):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        # Log human/expert and agent step counts so PVP intervention statistics
        # are always visible in training logs.
        human_steps = self.human_data_buffer.buffer_size if self.human_data_buffer.full else self.human_data_buffer.pos
        agent_steps = self.replay_buffer.buffer_size if self.replay_buffer.full else self.replay_buffer.pos
        total_steps = human_steps + agent_steps
        self.logger.record("train/human_expert_steps", human_steps)
        self.logger.record("train/agent_steps", agent_steps)
        self.logger.record("train/total_collected_steps", total_steps)
        if total_steps > 0:
            self.logger.record("train/human_expert_ratio", human_steps / total_steps)

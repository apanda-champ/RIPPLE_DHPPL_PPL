...
class ExpertTakeoverEnv(DrivingEnv):
    ...
    def store_preference_pairs(self, predicted_traj, preference_horizon, expert_action):
        # Determine the actual number of steps we are iterating over
        L = min(len(predicted_traj) - 1, preference_horizon)
        
        for step in range(L):
            # Calculate a linear decay rate. 
            # At step 0, decay_rate = 0.0. At step L-1, decay_rate = 1.0
            decay_rate = step / (L - 1) if L > 1 else 0
            
            # Scale factor decays from 1.0 down to a minimum of 0.2
            # You can adjust the 0.8 multiplier to change the minimum scale 
            scale_factor = 1.0 + (decay_rate * 0) 
            
            # Create a fresh copy of the expert action for this specific step
            scaled_expert_action = expert_action.copy()
            
            # --- ISOLATED STEERING SCALING ---
            # action[0] is steering, action[1] is acceleration.
            # Apply the scale factor ONLY to the steering component.
            scaled_expert_action[0] = scaled_expert_action[0] * scale_factor
            # ---------------------------------
            
            # CRITICAL: Clip the action using the environment's defined bounds
            scaled_expert_action = np.clip(
                scaled_expert_action, 
            ...
            step_info = {
                ...
                "action": scaled_expert_action.copy(),  # Inject the modified action
